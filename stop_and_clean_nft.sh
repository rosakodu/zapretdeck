...
reset_dns() {
    if [[ -x "$DNS_SCRIPT" ]]; then
        log "Сброс настроек DNS через dns.sh..."
        if sudo bash "$DNS_SCRIPT" unset >> "$LOG_FILE" 2>&1; then
            log "DNS возвращён к настройкам провайдера (DHCP)."
        else
            log "Заметка: dns.sh вернул ошибку (возможно, DNS не был изменен)."
        fi
    else
        log "Файл dns.sh не найден, пропускаем шаг DNS."
    fi
}
