...
sudo mkdir -p /opt/zapretdeck/{custom-strategies,zapret-latest,i18n}
sudo cp -r "$TEMP_DIR"/* /opt/zapretdeck/
sudo chmod +x /opt/zapretdeck/{main_script.sh,stop_and_clean_nft.sh,service.sh,rename_bat.sh,nfqws,main.py,compile_translations.sh}
...
echo -e "${BLUE}Компиляция файлов переводов...${NC}"
...
if [ -n "$LRELEASE_CMD" ]; then
    for ts_file in /opt/zapretdeck/i18n/*.ts; do
        if [ -f "$ts_file" ]; then
            sudo $LRELEASE_CMD "$ts_file" -qm "${ts_file%.ts}.qm" 2>/dev/null || \
                echo -e "${WHITE}Предупреждение: не удалось скомпилировать $ts_file${NC}"
        fi
    done
    echo -e "${GREEN}Переводы скомпилированы${NC}"
else
    echo -e "${WHITE}Предупреждение: lrelease не найден, переводы не скомпилированы${NC}"
    echo -e "${WHITE}Установите qt6-tools для компиляции переводов${NC}"
    echo -e "${WHITE}Приложение будет работать с русским интерфейсом по умолчанию${NC}"
fi

# === 11. Запускной файл и ярлык ===
sudo bash -c "cat > /usr/local/bin/zapretdeck" << 'EOF'
#!/bin/bash
exec /opt/zapretdeck/venv/bin/python3 /opt/zapretdeck/main.py "$@"
EOF
