...
class StatusChecker(QThread):
    ...
    warp_status_changed = pyqtSignal(bool)
    warp_registration_changed = pyqtSignal(bool)
    ...
    def run(self) -> None:
        ...
        last_warp_status_check = 0
        last_warp_reg_check = 0
        
        while self._running:
            try:
                # 1. Check session (nfqws process) - High frequency
                ...
                # 2. Check service - High frequency
                ...
                # 3. WARP Status - Medium frequency (every 10s)
                now = time.time()
                if now - last_warp_status_check >= 10:
                    if warp.is_warp_installed():
                        is_connected, _ = warp.get_warp_status()
                        self.warp_status_changed.emit(is_connected)
                    last_warp_status_check = now

                # 4. WARP Registration - Low frequency (every 60s)
                if now - last_warp_reg_check >= 60:
                    if warp.is_warp_installed():
                        # Internal retries are handled inside verify_warp_registration
                        is_registered, _ = warp.verify_warp_registration(retries=2)
                        self.warp_registration_changed.emit(is_registered)
                    last_warp_reg_check = now
