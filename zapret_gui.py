...
GITHUB_REPO = "yourusername/zapretdeck"  # Замени на свой репозиторий (например, yourusername/zapretdeck)
...
class ZapretGUI(ctk.CTk):
    ...
    def get_latest_version(self):
        ...
        try:
            ...
            latest_version = release_data['tag_name'].lstrip('v')  # Убираем 'v' из тега (напр., v1.0.0 -> 1.0.0)
    ...
    def download_and_update(self, latest_version):
        ...
        try:
            # Формируем URL архива (предполагаем tar.gz, замени на zip, если нужно)
            archive_name = f"zapretdeck-{latest_version}.tar.gz"  # Адаптируй под имя твоего архива
            release_url = f"https://github.com/{GITHUB_REPO}/releases/download/v{latest_version}/{archive_name}"
            ...
            extracted_dir = os.path.join(temp_dir, f"zapretdeck-{latest_version}")  # Предполагаемая структура архива
            if os.path.exists(extracted_dir):
                # Перемещаем только содержимое zapret-latest
                zapret_latest_src = os.path.join(extracted_dir, "zapret-latest")
                if os.path.exists(zapret_latest_src):
                    subprocess.run(["sudo", "-S", "rm", "-rf", REPO_DIR], input=password + "\n", text=True, capture_output=True)
                    subprocess.run(["sudo", "-S", "mv", zapret_latest_src, REPO_DIR], input=password + "\n", text=True, capture_output=True)
                else:
                    raise Exception("Папка zapret-latest не найдена в архиве")
            else:
                raise Exception("Неправильная структура архива")

            # Устанавливаем права
            subprocess.run(["sudo", "-S", "chmod", "+x", f"{REPO_DIR}/nfqws"], input=password + "\n", text=True, capture_output=True)

            # Обновляем version.txt
            subprocess.run(["sudo", "-S", "sh", "-c", f"echo {latest_version} > {VERSION_FILE}"], input=password + "\n", text=True, capture_output=True)

            # Очищаем временные файлы
            shutil.rmtree(temp_dir)

            logger.info(f"Обновление до {latest_version} завершено")
            return True
        except Exception as e:
    ...
    def check_update(self):
        """Проверяет и применяет обновление."""
        self.session_button.configure(state="disabled")  # Блокируем кнопку сессии
        current_version = self.load_version()
        latest_version = self.get_latest_version()

        if not latest_version:
            status_label = ctk.CTkLabel(self.main_frame, text="Ошибка: Не удалось проверить обновления", text_color=COLOR_RED, font=("Inter", 11))
        ...
        if latest_version == current_version:
            status_label = ctk.CTkLabel(self.main_frame, text=f"Версия {current_version} актуальна", text_color=COLOR_GREEN, font=("Inter", 11))
            status_label.pack(pady=2)
        ...
        if messagebox.askyesno("Обновление доступно", f"Доступна версия {latest_version} (текущая: {current_version}).\nОбновить?"):
            status_label = ctk.CTkLabel(self.main_frame, text="Обновление...", text_color=COLOR_ACCENT, font=("Inter", 11))
            status_label.pack(pady=2)
            if self.download_and_update(latest_version):
                status_label.configure(text=f"Обновлено до {latest_version}! Перезагрузите приложение.", text_color=COLOR_GREEN)
                self.after(5000, lambda: self.load_strategies())  # Перезагружаем стратегии
            else:
                status_label.configure(text="Ошибка обновления. Проверьте логи.", text_color=COLOR_RED)
            self.after(5000, status_label.destroy)
        else:
            status_label = ctk.CTkLabel(self.main_frame, text="Обновление отменено", text_color=COLOR_GRAY, font=("Inter", 11))
            status_label.pack(pady=2)
            self.after(3000, status_label.destroy)
        self.session_button.configure(state="normal")
