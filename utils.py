...
def get_base_dir() -> str:
    ...
    # Use script location as primary (current working directory approach)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    if os.path.exists(script_dir) and os.path.isdir(script_dir):
        return script_dir
    # Fallback to $HOME/zapretdeck
    home_dir = os.environ.get("HOME", "")
    alt_path = os.path.join(home_dir, "zapretdeck")
    if os.path.exists(alt_path) and os.path.isdir(alt_path):
        return alt_path
    # Last resort: return script location even if check fails
    return os.path.dirname(os.path.abspath(__file__))


def get_installation_base_dir() -> str:
    """Determine the installation base directory for ZapretDeck.

    This function prioritizes the actual installation directory ($HOME/zapretdeck)
    over the script location to ensure compatibility with the installer.
    """
    # First check the standard installation location
    home_dir = os.environ.get("HOME", "")
    install_path = os.path.join(home_dir, "zapretdeck")
...
BASE_DIR = get_installation_base_dir()
...
BUYVPN_IMAGE_PATH = os.path.join(BASE_DIR, "ad", "buyvpn.png")
...
class ConfigManager:
    ...
    def save(self) -> None:
        ...
        try:
            with open(self.config_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"show_info={self._config.get('show_info', 'true')}\n")
    ...
    def load_show_info(self) -> bool:
        """Get show info state from config."""
        val = self.get("show_info", "true").lower()
        return val in ("true", "1", "yes", "on", "enabled")
    
    def save_show_info(self, enabled: bool) -> None:
        """Save show info state to config."""
        self.set("show_info", "true" if enabled else "false")
        self.save()
